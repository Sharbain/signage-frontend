...
export const api = {
  ...
  devices: {
    ...
    // ✅ FIXED: backend route is /api/admin/devices/:deviceId/command
    // (your DevTools screenshot shows this exact endpoint)
    command: async (deviceId: string, data: any) =>
      authorizedFetch(`${API_BASE}/admin/devices/${encodeURIComponent(deviceId)}/command`, {
        method: "POST",
        body: JSON.stringify(data),
      }),
  },
  ...
  admin: {
    ...
    screens: {
      rotateToken: async (id: number) => {
        const res = await authorizedFetch(`${API_BASE}/admin/screens/${id}/rotate-token`, {
          method: "POST",
        });
